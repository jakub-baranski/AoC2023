...
func part_one() {
	...
	for _, line := range strings.Split(inp, "\n") {
		...
		gameNo, _ := strconv.Atoi(strings.TrimSpace(parts[0][5:]))

		draws := strings.Split(parts[1], "; ")
		for _, draw := range draws {
			for _, color := range strings.Split(draw, ", ") {
				parts := strings.Split(color, " ")
				count, color := parts[0], parts[1]
		...
		if game_possible {
			fmt.Println("Game", gameNo, "is possible")
			possible_games = append(possible_games, int(gameNo))
		} else {
			fmt.Println("Game", gameNo, "is not possible")
		}

	}
	sum := 0
	for _, game := range possible_games {
		sum += game
	}
	print(sum, "\n")
}
	...
	for _, line := range strings.Split(inp, "\n") {
		...
		for _, draw := range draws {
			for _, color := range strings.Split(draw, ", ") {
				...
				case "green":
					if countInt > min_green {
						min_green = countInt
					}
				case "blue":
					if countInt > min_blue {
						min_blue = countInt
					}
		...
		gamePowers = append(gamePowers, min_red*min_green*min_blue)

		//fmt.Println("Game", gameNo, "has min red", min_red, "min green", min_green, "min blue", min_blue)
